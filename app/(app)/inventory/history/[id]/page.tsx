...
export default function TransactionHistoryPage() {
    ...
    const getTransactionTypeUI = (type: Transaction['transaction_type'], quantity: number) => {
        if (type === 'dispense') {
            return <Badge variant="destructive"><Minus className="h-3 w-3 mr-1"/>Dispensed ({quantity})</Badge>
        }
        if (type === 'receive') {
            return <Badge className="bg-green-100 text-green-800"><Plus className="h-3 w-3 mr-1"/>Received (+{quantity})</Badge>
        }
                ...
                <CardContent>
                    <div className="rounded-md border">
                        <Table>
                                ...
                                {loading ? (
                                    ...
                                    transactions.map(tx => (
                                        <TableRow key={tx.id}>
                                            ...
                                            <TableCell>{getTransactionTypeUI(tx.transaction_type, tx.quantity_change)}</TableCell>
                                            <TableCell className="text-right">{formatCurrency(tx.price_per_unit)}</TableCell>
                                            <TableCell className={`text-right font-semibold ${Number(tx.value_change) < 0 ? 'text-red-600' : 'text-green-600'}`}>
                                                {formatCurrency(Number(tx.value_change))}
                                            </TableCell>
                                            <TableCell className="text-center">{tx.user_name || 'System'}</TableCell>
                                            <TableCell className="text-center">{tx.notes || 'N/A'}</TableCell>
                                        </TableRow>
                                    ))
                                )}
                            </TableBody>
                            <TableFooter>
                                <TableRow>                                    
                                    <TableCell colSpan={1} className="font-bold text-right"></TableCell>
                                    <TableCell colSpan={2} className="font-bold text-right">Net Value Change</TableCell>
                                    <TableCell colSpan={1} className={`text-right font-bold ${totalValueChange < 0 ? 'text-red-600' : 'text-green-600'}`}>
                                        {formatCurrency(totalValueChange)}
                                    </TableCell>
                                    <TableCell colSpan={2} className="font-bold text-right"></TableCell>
                                </TableRow>
                            </TableFooter>
                        </Table>
                    </div>
